...
9. PASO 2, SALIR DE BUCLE Y GUARDAR: si y sólo si todo funciona correctamente AHORA MISMO, llamarás a `exit_loop(mas_name)` pasando como argumento el nombre del proyecto solicitado por el usuario para proceder al guardado automático y finalizar el desarrollo. Recuerda que `exit_loop` requiere obligatoriamente el parámetro `mas_name` con el nombre del proyecto en minúsculas. 
